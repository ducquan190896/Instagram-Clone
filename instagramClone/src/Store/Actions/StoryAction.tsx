...
export const getStoriesOfFollowingsAction = () => async (dispatch: Dispatch<ACTION>, getState: any) => {
    try {
        ...
        const res = await fetch(RootURL + "/api/stories/authUser/allByFollowings", {
            method: "GET",
            headers: {
                "Authorization": token ?? ""
            }
...
export const getStoryByidAction = (storyId: number) => async (dispatch: Dispatch<ACTION>, getState: any) => {
    try {
        const token = await AsyncStorage.getItem("token")
        const res = await fetch(RootURL + `/api/stories/story/${storyId}`, {
            method: "GET",
            headers: {
                "Authorization": token ?? ""
            }
...
export const deleteStoryAction = (storyId: number) => async (dispatch: Dispatch<ACTION>, getState: any) => {
    try {
        const token = await AsyncStorage.getItem("token")
        await fetch(RootURL + `/api/stories/authUser/deleteStory/${storyId}`, {
            method: "DELETE",
            headers: {
                "Authorization": token ?? ""
            }
...
export const createStoryAction = (storyForm: STORYFORM) => async (dispatch: Dispatch<ACTION>, getState: any) => {
    try {
        const token = await AsyncStorage.getItem("token")
        const res = await fetch(RootURL + "/api/stories/authUser/createStory", {
            method: "POST",
            headers: {
                "Authorization": token ?? "",
