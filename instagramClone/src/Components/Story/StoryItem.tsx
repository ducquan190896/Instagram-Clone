...
const StoryItem = ({story, index, navigation}: {story: STORY, index: number, navigation: any}) => {
  ...
  return (
    <TouchableOpacity onPress={navigateToStoryScreen}  style={tw(' mr-2 items-center justify-center')} >
        <Image style={[tw('w-14 h-14 rounded-full bg-white -z-10 '), {resizeMode: 'center'}]} source={story?.owner?.avatarUrl ? {uri: RootURL + story?.owner?.avatarUrl}: require("../../assets/download.png")}></Image>
