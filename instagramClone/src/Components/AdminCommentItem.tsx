...
const AdminCommentItem = ({item, setIsReply, setParentCommentId, setParent}: commentItemProps) => {
    ...
    <View>
        ...
        {childrenComments && childrenComments.length > 0 && (
            <View style={tw('w-full pl-6')}>
                {childrenComments.map((item: COMMENT) =>  <AdminCommentItem key={item.id} item={item} setIsReply={setIsReply} setParent={setParent} setParentCommentId={setParentCommentId}></AdminCommentItem>)}
            </View>
        )}
    </View>
  )
