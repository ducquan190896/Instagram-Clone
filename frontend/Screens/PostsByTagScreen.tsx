...
// type postByTagScreenRouteProp = RouteProp<RootStackParamList, "PostsByTagScreen">

const PostsByTagScreen = () => {
    // const {params} = useRoute<postByTagScreenRouteProp>()
    // const {tag} = params
    const tag = "hel"
    ...
    const loadPostBytag = useCallback(async ()=> {
        if(tag && tag.length > 0) {
          dispatch(getPostsByTag(tag) as any)
        }
    }, [tag])
