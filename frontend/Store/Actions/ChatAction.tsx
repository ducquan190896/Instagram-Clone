...
export const getAllChatsByAuthAction = () => async(dispatch: Dispatch<ACTION>, getState: any) => {
    try {
        ...
        console.log(token)
        const res = await fetch("http://10.0.2.2:8080/api/chats/byAuthUser", {
            method: "GET",
            headers: {
                "Authorization": token ?? ""
            }
...
export const getOrAddChatAction = (receiverId: number) => async(dispatch: Dispatch<ACTION>, getState: any) => {
    try {
        ...
        const token = await AsyncStorage.getItem("token")
        console.log(token)
        const res = await fetch(`http://10.0.2.2:8080/api/chats/receiver/${receiverId}`, {
            method: "GET",
            headers: {
                "Authorization": token ?? ""
            }
...
export const getChatByIdAction = (chatId: number) => async(dispatch: Dispatch<ACTION>, getState: any) => {
    try {
        ...
        const token = await AsyncStorage.getItem("token")
        console.log(token)
        const res = await fetch(`http://10.0.2.2:8080/api/chats/chat/${chatId}`, {
            method: "GET",
            headers: {
                "Authorization": token ?? ""
            }
