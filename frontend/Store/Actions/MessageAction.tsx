...
export const getMessagesOfChatAction = (chatId: number) => async(dispatch: Dispatch<ACTION>, getState: any) => {
    try {
        ...
        const res = await fetch(`http://10.0.2.2:8080/api/messages/chat/${chatId}`, {
            method: "GET",
            headers: {
                "Authorization": token ?? ""
            }
...
export const addMessageAction = (messageForm: MESSAGEFORM) => async(dispatch: Dispatch<ACTION>, getState: any) => {
    try {
        const token = await AsyncStorage.getItem("token")
        const res = await fetch("http://10.0.2.2:8080/api/messages/addMessage", {
            method: "POST",
            headers: {
                "Authorization": token ?? "",
...
export const removeMessageAction = (messageId: number) => async(dispatch: Dispatch<ACTION>, getState: any) => {
    try {
        const token = await AsyncStorage.getItem("token")
        await fetch(`http://10.0.2.2:8080/api/messages/removeMessage/${messageId}`, {
            method: "DELETE",
            headers: {
                "Authorization": token ?? ""
            }
