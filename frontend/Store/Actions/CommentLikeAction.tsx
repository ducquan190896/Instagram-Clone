...
export const getCommentLiksByCommentAction = (commentId: number) => async(dispatch: Dispatch<ACTION>, getState: any) => {
    try {
        ...
        console.log(token)
        const res = await fetch(`http://10.0.2.2:8080/api/commentLike/allByComment/${commentId}`, {
            method: "GET",
            headers: {
                "Authorization": token ?? ""
            }
...
export const addCommentLikeAction = (commentId: number, postId: number) => async(dispatch: Dispatch<ACTION>, getState: any) => {
    try {
        ...
        const token = await AsyncStorage.getItem("token")
        console.log(token)
        const res = await fetch(`http://10.0.2.2:8080/api/commentLike/likeComment/comment/${commentId}/post/${postId}`, {
            method: "POST",
            headers: {
                "Authorization": token ?? ""
            }
...
export const unlikeCommentAction = (commentId: number, ownerId: number) => async(dispatch: Dispatch<ACTION>, getState: any) => {
    try {
        ...
        const token = await AsyncStorage.getItem("token")
        console.log(token)
         await fetch(`http://10.0.2.2:8080/api/commentLike/removeLikeFromComment/comment/${commentId}`, {
            method: "DELETE",
            headers: {
                "Authorization": token ?? ""
            }
