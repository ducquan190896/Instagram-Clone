  ...
  return (
    ...
    {isAuthUser ? (
        ...
        <View style={[tw('w-2/3 my-2 px-4 flex flex-row items-center justify-end'), {alignSelf: 'flex-end'}]}>
            ...
            <Image style={[tw('w-8 h-8 rounded-full bg-white ml-2 mr-2'), {resizeMode: 'contain'}]} source={user.avatarUrl ? {uri: user.avatarUrl}: require("../assets/download.png")}></Image>
        </View>
        {isDelete && (
        ...
        <View style={[tw('w-2/3 my-2 px-2 flex flex-row items-center justify-start'), {alignSelf: 'flex-start'}]}>
            <Image style={[tw('w-8 h-8 rounded-full bg-white ml-2 mr-2'), {resizeMode: 'contain'}]} source={item?.participant.owner.avatarUrl ? {uri: item?.participant.owner.avatarUrl}: require("../assets/download.png")}></Image>
            <TouchableOpacity onPress={() => setIsDelete(!isDelete )} style={tw('p-2 rounded-lg bg-gray-300')}>
                <Text style={tw('text-base')}>{item.text}</Text>
            </TouchableOpacity>
