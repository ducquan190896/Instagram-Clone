  ...
  return (
    <View  style={tw('w-full border-b border-gray-300 flex-row items-center justify-between px-6 py-2')}>
        <View style={tw('items-center justify-center flex-row')}>
            <Image style={tw('w-14 h-14 rounded-full mr-4')} source={receiver?.avatarUrl ? {uri: receiver?.avatarUrl} : require("../assets/download.png")}></Image>
            <Text style={tw('text-lg font-bold')}>{receiver?.username}</Text>
        </View>
        <Button onPress={navigateToOtherUserHomeScreen} title="send message" containerStyle={tw('w-32 h-10')} buttonStyle={tw('rounded-lg')}></Button>
  </View>
  )
}
