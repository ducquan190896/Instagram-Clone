...
const StoryNotifyItem = ({item, authUser}: {item: STORYNOTIFY, authUser: USER})  => {
    const [creator, setCreator] = useState<USER>(item?.creator)
    ...
    if(item?.creator.id == authUser.id) {
        return null;
    }
  ...
  return (
    ...
    <View style={tw('flex-row items-center justify-start flex-1')}>
        ...
        <Text style={tw('text-base')}>like your story</Text>
...
export default StoryNotifyItem
