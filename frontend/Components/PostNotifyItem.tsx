...
import { postCommentType, postLikeType } from '../Screens/NotificationsScreen'
import { NativeStackNavigationProp } from '@react-navigation/native-stack'
import { RootStackParamList } from '../Navigators/MainStack'
import { useNavigation } from '@react-navigation/native';

const PostNotifyItem = ({item, authUser}: {item: POSTNOTIFY, authUser: USER}) => {
  ...
  return (
    <TouchableOpacity onPress={navigationToPost} style={tw('my-2 mx-2 flex-row items-center justify-center')}>
     <View style={[tw('rounded-full bg-red-400 mr-6'), {padding: 2}]}>
        <Image style={[tw('w-12 h-12 rounded-full bg-white'), {resizeMode: 'contain'}]} source={creator?.avatarUrl ? {uri: creator.avatarUrl}: require("../assets/download.png")}></Image>        
    </View>
    <View style={tw('flex-row items-center justify-start flex-1')}>
        ...
        <Text style={tw('text-base')}>{item?.type == postLikeType ? " like your post" : " comment on your post" }</Text>
