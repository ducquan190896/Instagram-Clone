...
const PollingComponent = ({postParams}: {postParams: Post}) => {
    ...
    const loadVotesByPoll = useCallback( async () => {
      try {
        ...
        const res = await fetch(`http://10.0.2.2:8080/api/votes/poll/${poll?.id}`, {
            method: "GET",
            headers: {
                "Authorization": token ?? "",
            }
    ...
    const AddVoteFunction = async () => {
        if(selectedChoice) {
            ...
            try {
              const token = await AsyncStorage.getItem("token")
              const res = await fetch(`http://10.0.2.2:8080/api/votes/addVote/poll/${poll?.id}/choice/${selectedChoice}`, {
                  method: "POST",
                  headers: {
                      "Authorization": token ?? "",
                  }
    ...
    const handleRenderItem: ListRenderItem<any> = ({item}) => (
           ...
           {isActivePoll && !isVoted ? (
             <TouchableOpacity onPress={() => setSelectedChoice(item?.id)} activeOpacity={0.5} style={tw(`mx-2 mb-2 z-10 rounded-lg   ${item?.id == selectedChoice ? "bg-blue-500" : "bg-gray-200"} relative`)}>
             <View style={[tw(' rounded-lg px-2 py-2  flex-row items-center justify-between')]}>
                 <Text style={tw('text-base ml-4')}>{item.answer}</Text>
                 <Text style={tw('text-base mr-4')}>{(item.voteCount * 100 / (totalVote > 1 ? totalVote : 1)).toFixed(0)} %</Text>
             </View>
  ...
  return (
    <View style={tw('w-full my-2 px-2')}>
      <Text style={tw('text-lg font-bold mb-4 text-center')}>{poll?.question}</Text>
      <FlatList
      data={poll?.choices}
      ...
      <View style={tw(' flex flex-row items-center justify-between px-2 mt-2')}>
        ...
        {isActivePoll && !isVoted && <Button onPress={AddVoteFunction} buttonStyle={tw('bg-blue-500 rounded-lg')} title="Submit Vote"></Button>}
        {!isActivePoll && (
            <Text  style={tw('text-lg font-bold')}> Poll expired</Text>
        )}
         {isVoted && (
            <Text  style={tw('text-lg font-bold bg-blue-500 p-2 rounded-lg text-white')}> Voted Successfully</Text>
        )}
        <Text  style={tw('text-lg font-bold')}>{totalVote} Votes</Text>
      </View>
    </View>
  )
}
