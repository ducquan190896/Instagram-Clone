...
const CommentItem = ({item, setIsReply, setParentCommentId, setParent}: commentItemProps) => {
   const [isLike, setIsLike] = useState<boolean>(false)
    ...
    const loadLikeCommentStatusByAuth = useCallback( async () => {
        ...
        console.log(token)
        const res = await fetch(`http://10.0.2.2:8080/api/commentLike/checkCommentLikeByAuthUser/${item?.id}`, {
            method: "GET",
            headers: {
                "Authorization": token ?? ""
            }
...
<View>
    <View style={tw('w-full px-2 flex-row items-center justify-between mb-4')}>
         <Image style={[tw('w-12 h-12 rounded-full bg-white mr-4'), {resizeMode: 'contain'}]} source={item?.ownerResposne?.avatarUrl ? {uri: item?.ownerResposne?.avatarUrl}: require("../assets/download.png")}></Image>
